...
/**
   常用命令
    ...
    4）对value操作的命令
    exists(key)：确认一个key是否存在   eg：exists name
    del(key)：删除一个key
    type(key)：返回值的类型
    ...
    ttl      获得一个key的活动时间
    select  db-index  按索引查询 数据库标号是从0开始的
    ...
    mset(key N, value N)：批量设置多个string的值 eg： mset color1 red color2 blue
    mget(key1, key2,…, key N)：返回库中多个string的value
    incr(key)：名称为key的integer增1操作   如果没有 则创建一个新的 为 1
    decr(key)：名称为key的integer减1操作   如果没有 则创建一个新的 为 -1
    incrby(key, integer)：名称为key的string增加integer
    decrby(key, integer)：名称为key的string减少integer
    append(key, value)：名称为key的string的值附加value
    substr(key, start, end)：返回名称为key的string的value的子串 包括开始位置和结束位置
    ...
    ltrim(key, start, end)：截取名称为key的list ，保留指定区域的元素，开始结束均保留
...
　　　　　　　　　　　　　　并将该元素添加到名称为dstkey的list的头部
    ...
    8）sorted set
    zadd key score member：向key的set中加入权值的member eg： zadd hotmessage 203 15
    zrem key member ：删除指定元素，1成功 
    zincrby key incr member ： 按照incr幅度增加对应的member的score值，返回score值
    zrange key start end： 为key的权值正序排序  start和end为第n个开始和结束的元素
    zrevrange key start end ：为key的域权值逆序  start和end为第n个开始和结束的元素
    zcard key ：返回集合中元素的个数
    zscore key element ：返回指定元素的对应的score
    zrank key member ：返回指定元素在集合中的排名，按照score从小到大的顺序排列
    zrevrank key member ： 返回指定元素在集合中的排名，按照score从大到小的顺序排列
    zremrangebyrank key min max  ： 删除集合中指定区间内的元素 （权值由小到大） 开始结束元素都包括
